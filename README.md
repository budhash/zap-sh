...
> Generate a script live with the interactive wizard. It runs the same generator
> as the CLI (a JavaScript port kept byte-for-byte identical to `zap-sh init` —
> see [JavaScript generator](#javascript-generator)), entirely client-side.
...
`zap-sh init` also has a first-class JavaScript implementation,
[`@budhash/zap-sh`](packages/generator), that produces the **exact same script**
as the bash tool. It powers the [browser wizard](https://budhash.com/zap-sh/) and
can be used directly in Node or the browser.
...
The bash script is the reference oracle. Two things keep the JS implementation
from ever drifting:

- **[`SPEC.md`](SPEC.md)** — a versioned contract describing exactly what
  `zap-sh init` emits (sections, variable order, substitution rules, licenses,
  determinism).
- **[`test/conformance/`](test/conformance)** — shared fixtures plus golden files
  generated by the real `zap-sh init`. **Both** a bash runner and the JS runner
  check every fixture in CI, so bash and JS output must match byte-for-byte.
