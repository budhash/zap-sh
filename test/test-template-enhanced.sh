...
test_piped_execution() {
  ...
  # Create a test script from enhanced template
  local test_script="test-piped-enhanced.sh"
  
  # Test 1: Basic piped execution - template should run when piped
  cp "$TEMPLATE_FILE" "$test_script"
  chmod +x "$test_script"
  
  local output
  output=$(cat "$test_script" | bash -s -- -h 2>&1 || true)
  assert_ok test $? -eq 0 "enhanced script executes when piped"
  
  # Test 2: Verify __PIPED detection in a minimal script
  cat > "$test_script" << 'EOF'
#!/usr/bin/env bash
# Minimal test for piped mode detection - based on enhanced template metadata
...
EOF
  ...
  assert_contains "$output" "NORMAL_MODE" "enhanced normal execution detects non-piped mode"
  
  # Test piped execution
  output=$(cat "$test_script" | bash 2>&1 || true)
  assert_contains "$output" "PIPED_MODE_DETECTED" "enhanced piped execution detects piped mode"
  
  # Test that enhanced template's argument parsing works in piped mode
  # Create a minimal test that doesn't run the full template
  cat > "$test_script" << 'EOF'
#!/usr/bin/env bash
# Test metadata section only
set -euo pipefail; IFS=$'\n\t'
...
EOF
  ...
  output=$(cat "$test_script" | bash -s -- arg1 arg2 2>&1)
  # Arguments after -- are passed correctly
  assert_contains "$output" "arg1 arg2" "enhanced template passes arguments in piped mode"
  assert_contains "$output" "PIPED: true" "enhanced template detects piped mode"
  
  # Test that metadata variables are set correctly
  cat > "$test_script" << 'EOF'
#!/usr/bin/env bash
...
EOF
  ...
  assert_contains "$output" "APP=bash" "enhanced __APP fallback to bash in piped mode"
  assert_contains "$output" "APPFILE=" "enhanced __APPFILE empty in piped mode"
  assert_contains "$output" "PIPED=true" "enhanced __PIPED detected correctly"
