...
## Homebrew tap distribution (1.1.2)

Shipped `brew install budhash/tools/zap-sh` via the shared tap
`budhash/homebrew-tools` (formulae in `Formula/*.rb`). gomanize (Go) does this
with GoReleaser's `brews:`; zap-sh is a bash tool with no GoReleaser, so it uses
a **script formula** generated by a `homebrew` job in `release.yml`.

- **What the job does:** downloads the tag's GitHub source tarball
  (`/archive/refs/tags/vX.Y.Z.tar.gz`), `sha256sum`s it, writes
  `Formula/zap-sh.rb` (`bin.install "zap-sh"`, `test do` runs the version flag),
  and pushes it to the tap via `HOMEBREW_TAP_TOKEN` (fine-grained PAT, Contents
  RW on the tap — the built-in `GITHUB_TOKEN` can't reach another repo).
- **Non-fatal, ordered last:** `needs: [release]` + `continue-on-error: true`.
  A tap-push failure never fails the GitHub release or the independent npm
  publish (`release-npm.yml`). Missing token fails only this job, loudly.
- **Standalone install just works:** `_version()` reads `# VERSION:` from
  `$__APPFILE` (= `${BASH_SOURCE[0]}`), which follows Homebrew's `bin/` symlink,
  so `zap-sh -v` prints the version with only the bare script installed;
  templates auto-download to `~/.config/zap-sh` on first `init` (writes to
  `$HOME`, not the read-only Cellar).
- **Gotcha — no `--version`:** zap-sh parses flags with `getopts "hv"`, so the
  version flag is **`-v`**; `--version` errors (exit 2). The formula test and the
  README verify block both use `-v`/`-h`. (The README previously showed the
  non-working `--version`/`--help`.)
- **Formula heredoc in YAML:** emit the `.rb` with `<<'RUBY'` (no shell
  expansion — `#{...}` Ruby interpolation stays literal); `${{ ... }}` GitHub
  substitution still fills url/sha/version because it runs before the shell.
- **Tap URL sha256:** GitHub's auto-generated source tarballs are stable, so
  hashing the tarball in CI right after the tag matches what users download.
- **Versioning:** kept sh + js in lockstep at 1.1.2 (precedent: 1.1.1 was a
  build-only npm republish). `make bump-version` syncs only `version.txt` +
  `zap-sh`; the npm `package.json`/lock must be bumped separately
  (`npm version --no-git-tag-version`), else `release-npm.yml`'s guard skips the
  publish (tag ≠ package version).
